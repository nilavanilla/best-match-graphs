...
class Tree	{
	public:
		...
		void connect_trees(Tree subtree);		//line 20

		void build_hasse(std::vector<std::vector<int> > p, std::vector<Class> classes, int a, bool iN);	//line 49
		int compute_par(std::vector<int> &x, std::vector<std::vector<int> > p);	//line 125
		bool check_tree(std::vector<std::set<int> > r, std::vector<Vertex> v);	//line 147

		void find_leafSets(int n, std::vector<int> &subgraphs, std::vector<std::set<int> > &leafs, bool v);	//line 227
		int find_leafs(std::vector<int> subgraph, int n);	//line 253
		int pos_of_Vert(int n, std::vector<Vertex> v);	//lien 265

		bool build_aho(std::set<int> leafs, std::vector<std::vector<int> > triples, int p, std::vector<Vertex> vert, bool e);	//line 277

		void matrix_to_vector(std::vector<std::vector<int> > adj, std::vector<int> spec, std::vector<int> nr, std::vector<std::string> names, std::vector<Vertex> &vert, int nrIN);	//line 353
};
